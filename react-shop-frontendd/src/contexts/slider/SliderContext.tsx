...
export const SliderProvider: React.FC<Props> = ({ children }) => {
	...
	const createSlide: CreateSlide = (payload) => {
		return axios.post(`${requestUrl}/slider`, payload).catch((error) => {
			console.log(error);
		});
	};

	const editSlide: EditSlide = (id, payload) => {
		return axios.patch(`${requestUrl}/slider/${id}`, payload).catch((error) => {
			console.log(error);
		});
	};

	const deleteSlide: DeleteSlide = (id) =>
		axios.delete(`${requestUrl}/slider/${id}`);
