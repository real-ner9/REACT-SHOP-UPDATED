	...
	return (
		...
		>
			<div className="filters-form__header">
				...
				<p className="filters-form__reset" onClick={handleReset}>
					Очистить
				</p>
			</div>
			...
			<FormProvider key={key} {...methods}>
				...
				<ColorFilter
					defaultValue={formatArrayFromUrl(searchParams.get("color"))}
				/>

				<AmountFilter
					defaultValue={formatArrayFromUrl(searchParams.get("amount"))}
				/>
			...
			{mobile && (
				<StyledButton fullWidth onClick={handleClose}>
					Показать
				</StyledButton>
			)}
		</Paper>
