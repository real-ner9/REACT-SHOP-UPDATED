...
const checkoutSchema = object({
	...
	phone: string({ required_error: validationErrors.required("телефон") }).regex(
		new RegExp("^((8|\\+7)[\\- ]?)?(\\(?\\d{3}\\)?[\\- ]?)?[\\d\\- ]{7,10}$"),
		"Телефон указан неверно",
	),
	// address
	region: string({
		required_error: validationErrors.required("регион"),
	}).nonempty(validationErrors.required("регион")),
	city: string({ required_error: validationErrors.required("город") }).nonempty(
		validationErrors.required("город"),
	),
	street: string({
		required_error: validationErrors.required("улица"),
	}).nonempty(validationErrors.required("улица")),
	house: string({ required_error: validationErrors.required("дом") }).nonempty(
		validationErrors.required("дом"),
	),
	apartment: string({
		required_error: validationErrors.required("квартира"),
	}).nonempty(validationErrors.required("квартира")),
	comment: string()
		.max(128, validationErrors.max("комментарий", 128))
		.optional(),
});
	...
	return (
		...
		>
			...
			<FormProvider {...methods}>
				...
				<Stack
					className="checkout-form__delivery"
					direction={mobile ? "column" : "row"}
					spacing={2}
				>
